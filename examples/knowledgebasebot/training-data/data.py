from parametrize import parametrize, parametrize_all, randomize
from to_soloist_format import to_soloist_format
import json

x = [
    randomize( {"cuisine": random_cuisine}, 2, [
        {
            "user": "I could really go for some %(cuisine)s food right now",
            "belief": "asdfasdfasdf",
            "system": ""
        }
    ])
...
_ask_for_attribute_by_index = parametrize({
    ...
    {
        "user": [
            "What is the %(attribute)s of the %(index_text)s one?",
            "What's the %(attribute)s of the %(index_text)s?",
            "Tell me the %(attribute)s of the %(index_text)s %(domain)s",
            "Can you lookup the %(attribute)s of the %(index_text)s %(domain)s for me please?",
            "I want to know the %(attribute)s of the %(index_text)s"
        ]
    }
)
...
ask_for_restaurant_with_cuisine = parametrize({
    ...
    },
    {
        "user_input": [
            ...
            "I want to eat at a %(cuisine)s restaurant",
            "Can you list some %(cuisine)s restaurants for me?",
            "Please recommend some %(cuisine)s food places",
            "I love %(cuisine)s cuisine. Which restaurants offer that?",
            "Do you know a good place for a %(cuisine)s meal?"
        ]
    })
...
data = [
    *parametrize_all(ask_for_museum_about, {
        "about": ['art', 'history', 'war', 'cars']}, max_len=30),
    *parametrize_all(ask_for_restaurant_with_cuisine, {
        "cuisine": ["mexican", "indian", "german", "swedish", "chinese", "american", "italian"]}, max_len=30),
